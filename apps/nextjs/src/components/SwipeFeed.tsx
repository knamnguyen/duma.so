...
import { mockActivities, mockUsers, currentUser, type ActivityPost } from '~/lib/mockData';
...
export const SwipeFeed = () => {
  ...
  const swipedSet = useMemo(() => new Set(swipedActivityIds), [swipedActivityIds]);
  ...
  const generateMockHostData = (hostId: string, activityType: string) => {
    ...
    const professions = [
      ...
      "UX Researcher", "Content Creator", "Startup Founder", "Lawyer", "Chef",
      "Photographer", "Architect", "Personal Trainer", "Journalist", "DevOps Engineer"
    ];
  ...
  const realHostUsers = realActivities.map((activity) => {
    ...
    return {
      id: activity.host.id,
      name: `${activity.host.firstName || ""} ${activity.host.lastName || ""}`.trim() || "User",
  ...
  const currentHost = currentActivity ? allUsers.find(u => u.id === currentActivity.hostUserId) : null;
