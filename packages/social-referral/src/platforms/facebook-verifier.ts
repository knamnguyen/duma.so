...
const DEFAULT_ACTOR_ID = "kbzX2pUZc7cRZIwZc";
// const FACEBOOK_URL_REGEX =
//   /https?:\/\/(?:www\.)?facebook\.com\/(?:share\/(?:p|v)|reel)\/([A-Za-z0-9_-]+)\/?/i;
...
const isValidFacebookItem = (item: unknown): item is FacebookDatasetItem => {
  ...
  const content = datasetItem.content;
  if (typeof content !== "string") return false;
  return content.trim().length > 0;
};

/**
 * Verifies keywords in Facebook posts and videos using Apify scraper
 *
 * @example
 * const verifier = new FacebookVerifier();
 * const result = await verifier.verifyKeywords({
 *   url: "https://www.facebook.com/share/p/19zP8j8isb/",
 *   keywords: ["grind", "saturday"],
 * });
 */
export class FacebookVerifier implements SocialVerifier {
  ...
  async verifyKeywords({
    ...
    const content = validItem.content;
